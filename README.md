...
⚠️ **首次运行注意**：
第一次上传图片时，服务器会自动下载 AI 模型（约 300MB）。请确保网络连接正常（已配置国内镜像源，无需梯子）。模型下载后会缓存在项目根目录的 `.models` 文件夹下。

### 5. 运行测试
本项目使用 Vitest 进行 API 集成测试：
```bash
# 运行所有测试
bun run test
...
测试覆盖以下模块：
- `tests/api/auth.test.ts` - 用户认证（注册/登录/登出）
- `tests/api/posts.test.ts` - 帖子 CRUD
- `tests/api/claims.test.ts` - 认领流程
...
3.  **为什么使用 Zero-Shot Classification 而非 Image Captioning？**

    | 对比项 | Zero-Shot Classification | Image Captioning |
    |-------|------------------------|------------------|
    | 模型大小 | 较小（CLIP ~150MB） | 较大（通常 >1GB） |
    | 速度 | 快 | 慢 |
    | 结果可控性 | 输出固定在标签范围内 | 可能输出任意描述 |
    | 搜索一致性 | 便于标签匹配 | 需额外处理 |
    
    对于失物招领场景，使用预定义标签可以确保：
    - **搜索一致性**：用户搜索"钱包"时，能精确匹配所有被标记为"钱包"的帖子。
    - **多语言支持**：标签可统一翻译（英文识别 → 中文显示）。
    - **服务器友好**：轻量模型可在普通服务器运行，无需 GPU。
